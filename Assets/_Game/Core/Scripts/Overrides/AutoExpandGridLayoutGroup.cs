...
namespace UnityEngine.UI
{
    ...
    public class AutoExpandGridLayoutGroup : LayoutGroup
    {
        ...
        public override void CalculateLayoutInputHorizontal()
        {
            ...
            // 1. Calculate the base cellSize X based on children's preferred sizes
            float maxChildWidth = 0;
            for (int i = 0; i < rectChildren.Count; i++)
            {
                maxChildWidth = Mathf.Max(maxChildWidth, LayoutUtility.GetPreferredSize(rectChildren[i], 0));
            }
            // Optional: Provide a minimum fallback if maxChildWidth is 0
            m_CellSize.x = Mathf.Max(10, maxChildWidth); 
        ...
        {
            // 2. Calculate the base cellSize Y based on children's preferred sizes
            float maxChildHeight = 0;
            for (int i = 0; i < rectChildren.Count; i++)
            {
                maxChildHeight = Mathf.Max(maxChildHeight, LayoutUtility.GetPreferredSize(rectChildren[i], 1));
            }
            m_CellSize.y = Mathf.Max(10, maxChildHeight);
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            // Calculate what the size would be based on the INTENDED column/row counts (cellCount), 
            // not just the actual children present. This fixes the "8 children stretching across 10 columns" issue.
            int divisorX = Mathf.Max(1, cellCountX);
            int divisorY = Mathf.Max(1, cellCountY);
            
            float calculatedSizeX = ((availableWidth - (spacing[0] * (divisorX - 1))) / divisorX);
            float calculatedSizeY = ((availableHeight - (spacing[1] * (divisorY - 1))) / divisorY);

            // Apply expansion conditionally based on the new toggles
            float realsizeX = m_ChildForceExpandWidth ? Mathf.Max(cellSize.x, calculatedSizeX) : cellSize.x;
            float realsizeY = m_ChildForceExpandHeight ? Mathf.Max(cellSize.y, calculatedSizeY) : cellSize.y;

            Vector2 requiredSpace = new Vector2(
                actualCellCountX * realsizeX + (actualCellCountX - 1) * spacing.x,
                actualCellCountY * realsizeY + (actualCellCountY - 1) * spacing.y
                );
