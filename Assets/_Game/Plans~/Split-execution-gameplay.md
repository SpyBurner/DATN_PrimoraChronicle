...
| F2.3 | **Granted-cards shuffle + opening hand** | §3 Granted, §5 Start | `PlayerCardZoneNetworkView.ServerSetupDeckForMatch(championId, supportCardIds)` — reads `CardLoadingManagerSubsystem.GetCardData(championId).grants_cards`, shuffles into the 20 supports, deals 6 via `ServerDraw(6)`. HP initialization (`champion.hp`) is done here and forwarded to `PlayerRosterPublicNetworkView` via `GameplayNetworkCoordinator.Instance.GetPlayerRosterView(Owner)?.SendHPChangedRpc(...)`. |
