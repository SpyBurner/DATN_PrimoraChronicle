...
| `_fuseSlots[0].Root` | `FuseSlot0` GameObject | ⬜ |
| `_fuseSlots[0].NameText` | `FuseSlot0/NameText` TMP_Text | ⬜ |
| `_fuseSlots[0].Icon` | `FuseSlot0/Icon` Image | ⬜ |
| `_fuseSlots[0].ClearButton` | `FuseSlot0/ClearButton` Button | ⬜ |
| `_fuseSlots[1]` – `_fuseSlots[3]` | Repeat pattern for FuseSlot1–FuseSlot3 | ⬜ |
...
| Spec requirement | Status | Notes |
|---|---|---|
| Champion card pinned to base slot pool (not consumed from hand) | ⬜ **NOT IMPLEMENTED** | `FusionPanel` has no logic to pin or display the Champion card separately. `StageBase(cardId)` is driven by drag-drop from `HandPanel`. The Champion is treated as a regular hand card. A dedicated Champion display must be added to `FusionPanel` that reads `IPlayerCardZoneSubsystem.ChampionId` (or equivalent) and pre-populates `_baseCardId` without consuming the hand slot. |
...
`FuseSlotDropTarget.cs` handles drag-drop from `HandPanel` → FuseSlot. It must be attached to
each `FuseSlot[0-3].Root` GameObject.

| Field on `FuseSlotDropTarget` | Assign | Status |
|---|---|---|
| `_fusionPanel` | Parent `FusionPanel` MonoBehaviour | ⬜ |
| `_slotIndex` | 0, 1, 2, or 3 respectively | ⬜ |
...
| 7 | **Missing feature** | `FusionPanel` | Champion card not pinned to base slot — treated as normal hand card. |
